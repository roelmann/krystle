 ...
 * @return string
 */
function krystle_performance_output($param) {
 ...
 * @return string
 */
function krystle_process_css($css, $theme) {
    ...
    $css = krystle_set_backgroundcolor($css, $backgroundcolor);
    ...
    $css = krystle_set_logourl($css, $logourl);
    ...
    $css = krystle_set_menubackgroundcolor($css, $menubackgroundcolor);
    ...
    $css = krystle_set_customcss($css, $customcss);
 ...
 * @return string
 */
function krystle_set_backgroundcolor($css, $backgroundcolor) {
...
function krystle_set_logourl($css, $logourl) {
    global $OUTPUT;
    $tag = '[[setting:logourl]]';
    if (is_null($logourl)) {
 ...
 * @return string
 */
function krystle_set_menubackgroundcolor($css, $menubackgroundcolor) {
 ...
 * @return string
 */
function krystle_set_customcss($css, $customcss) {
 ...
 *     theme/krystle/yui/editbuttons/editbuttons.js
 *
 * @param moodle_page $page 
 */
function krystle_initialise_editbuttons(moodle_page $page) {
    $page->requires->string_for_js('edit', 'moodle');
    $page->requires->yui_module('moodle-theme_krystle-editbuttons', 'M.theme_krystle.initEditButtons');
}

function krystle_initialise_awesomebar(moodle_page $page) {
    ...
    $page->requires->yui_module('moodle-theme_krystle-awesomebar', 'M.theme_krystle.initAwesomeBar');
}

function krystle_require_course_login($courseorid, $autologinguest = true, $cm = NULL, $setwantsurltome = true, $preventredirect = true) {
    global $CFG, $SITE;
    ...
    if (!empty($CFG->forcelogin)) {
        ...
        krystle_require_login($courseorid, $autologinguest, $cm, $setwantsurltome, $preventredirect);
...
function krystle_require_login($courseorid = NULL, $autologinguest = true, $cm = NULL, $setwantsurltome = true, $preventredirect = false) {
...
class krystle_expand_navigation extends global_navigation {
    ...
    public function expand($branchtype, $id) {
        global $CFG, $DB, $PAGE;
        static $krystle_course_activities = array();
            ...
            case self::TYPE_SECTION :
                ...
                try {
                    ...
                    if (property_exists($PAGE->theme->settings, 'expandtoactivities') && $PAGE->theme->settings->expandtoactivities) {
                        if(!array_key_exists($course->id, $krystle_course_activities)) {
                            list($sectionarray, $activities) = $this->generate_sections_and_activities($course);
                            $krystle_course_activities[$course->id] = $activities;
                        }
                        $sections = $this->expandedcourses[$course->id];
                        $activities = $krystle_course_activities[$course->id];
    ...
    protected function load_section_activities(navigation_node $sectionnode, $sectionnumber, array $activities, $course = null) {
        ...
        foreach ($activities as $activity) {
            ...
            // Legacy onclick removed from krystle - clicking in Awesomebar should go to the page, not trigger popups etc.
